...
export const connectSocket=(session:Session):VttSocket=>{
  const socket=io({auth:{roomId:session.roomId,sessionToken:session.sessionToken},transports:['websocket','polling'],reconnection:true}) as VttSocket;
  socket.on('connect_error',(error)=>{
    if(/invalid or expired room session/i.test(error.message)){
      clearSession();
